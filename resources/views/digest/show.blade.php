    ...
    <section>
        ...
        <div>
            <div class="btn-group grid grid-cols-2 mt-6">
                <a href="{{ $previous ? route('digest.show', ['slug' => $previous->slug]) : '' }}"
                    aria-disabled="{{ $previous ? 'false' : 'true' }}"
                    class="btn btn-outline {{ $previous ? '' : 'pointer-events-none opacity-50' }}">Previous</a>
                <a href="{{ $next ? route('digest.show', ['slug' => $next->slug]) : '' }}"
                    aria-disabled="{{ $next ? 'false' : 'true' }}"
                    class="btn btn-outline {{ $next ? '' : 'pointer-events-none opacity-50' }}">Next</a>
            </div>
        </div>
    </section>
@endsection
