...
@can('create-digest', $digest)
    <div class="flex justify-around my-4 items-center">
        ...
        <label for="confirm-delete" class="btn bg-red-400 hover:bg-red-500 text-white">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <label for="confirm-delete" class="modal cursor-pointer">
            <label class="modal-box relative" for="">
                ...
                <form hx-delete="{{ route('digest.destroy', ['slug' => $digest->slug]) }}">
                    @csrf
                    <button type="submit" class="btn bg-red-400 text-white hover:bg-red-500 float-right mt-4">Yes</button>
                </form>
            </label>
        </label>
...
<article class="mt-4 is-loading-digest loading-digest">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('home', ['category' => $digest->category->name]) }}"
